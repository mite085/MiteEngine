 ...
 * 提取纹理变换参数（缩放/偏移）
 */
void ExtractTextureTransform(aiMaterial *aiMat, aiTextureType textureType,
                             unsigned int textureIndex, glm::vec2 &scale,
                             glm::vec2 &offset) {
  // 重置为默认值
  scale = glm::vec2(1.0f);
  ...
  if (aiMat->Get(AI_MATKEY_UVTRANSFORM(textureType, textureIndex),
...
std::vector<MaterialAssetID> MaterialLoader::LoadMaterialsFromGLTF(
  ...
  for (unsigned int i = 0; i < scene->mNumMaterials; i++) {
    ...
    if (materialID.IsValid()) {
      ...
      if (materialAsset) {
        LOG_INFO("Successfully loaded material: " +
                 materialAsset->metadata.name);

        // 创建纹理解析器回调函数（TODO:
        // 这里已经能正常获取到TextureInstance了，无需使用回调函数）
        auto textureResolver =
            [&textureCache](const TextureAssetID &id) -> TextureInstance {
        ...
        MaterialSourceData sourceData =
            materialAsset->metadata.generateSourceData(textureResolver);
...
MaterialAssetID MaterialLoader::ProcessGLTFMaterial(
    MaterialCache &materialCache, TextureCache &textureCache, aiMaterial *aiMat,
    uint32_t materialIndex, const std::string &modelPath,
    const aiScene *scene) {
  if (!aiMat) {
  ...
  std::string materialName =
      GenerateMaterialName(aiMat, materialIndex, modelPath);
  ...
  // 提取并创建纹理引用
  ExtractAndCreateTextureReferences(textureCache, aiMat,
                                    materialAsset->metadata, modelPath, scene);
...
void MaterialLoader::ExtractPBRParameters(aiMaterial *aiMat,
  ...
  if (aiMat->Get(AI_MATKEY_COLOR_EMISSIVE, color) == AI_SUCCESS) {
    metadata.parameters[MaterialParamKeys::EMISSION_COLOR] =
...
void MaterialLoader::ExtractAndCreateTextureReferences(
    TextureCache &textureCache, aiMaterial *aiMat, MaterialMetadata &metadata,
  ...
  if (aiMat->GetTexture(aiTextureType_NORMALS, 0, &texturePath) == AI_SUCCESS) {
    ...
    if (texId.IsValid()) {
      MaterialTextureSlot slot(texId, TextureTarget::TEXTURE_2D);
      ExtractTextureTransform(aiMat, aiTextureType_NORMALS, 0, slot.scale,
                              slot.offset);
  ...
  // 自发光纹理（默认2D纹理）
  if (aiMat->GetTexture(aiTextureType_EMISSIVE, 0, &texturePath) ==
      AI_SUCCESS) {
    TextureAssetID texId = CreateOrGetTextureAssetID(
        textureCache, texturePath.C_Str(), modelPath, scene);
    if (texId.IsValid()) {
      MaterialTextureSlot slot(texId, TextureTarget::TEXTURE_2D);
      ExtractTextureTransform(aiMat, aiTextureType_EMISSIVE, 0, slot.scale,
                              slot.offset);
  ...
  // 环境光遮蔽纹理（默认2D纹理）
  if (aiMat->GetTexture(aiTextureType_AMBIENT_OCCLUSION, 0, &texturePath) ==
      AI_SUCCESS) {
    TextureAssetID texId = CreateOrGetTextureAssetID(
        textureCache, texturePath.C_Str(), modelPath, scene);
    if (texId.IsValid()) {
      MaterialTextureSlot slot(texId, TextureTarget::TEXTURE_2D);
      ExtractTextureTransform(aiMat, aiTextureType_AMBIENT_OCCLUSION, 0,
                              slot.scale, slot.offset);
...
TextureAssetID MaterialLoader::CreateOrGetTextureAssetID(
  ...
  if (TextureLoader::IsEmbeddedTexturePath(texturePath)) {
    ...
    if (scene) {
      ...
      if (textureIndex >= 0 && textureIndex < (int)scene->mNumTextures) {
        ...
        return TextureLoader::LoadEmbeddedTexture(textureCache, texturePath,
                                                  modelPath, aiTex);
      }
    }
  } else {
