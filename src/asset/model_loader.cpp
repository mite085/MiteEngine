...
ModelAssetID ModelLoader::LoadGLTFModel(ModelCache &modelCache,
                                        ...
                                        const std::string &path, bool flipUVs,
                                        bool generateLODs,
  ...
  // GLTF特定优化
  flags |=
      aiProcess_ImproveCacheLocality;  // GLTF已经优化过，可以跳过一些预处理
  ...
  return LoadModelInternal(modelCache, materialCache, textureCache, scene, path,
                           generateLODs, lodLevels);
}
...
ModelAssetID ModelLoader::LoadModelInternal(
    ModelCache &modelCache, MaterialCache &materialCache,
    TextureCache &textureCache, const aiScene *scene, const std::string &path,
    bool generateLODs, const std::vector<float> &lodLevels) {
  // 检查缓存
  ...
  std::vector<MaterialAssetID> materialIDs =
      MaterialLoader::LoadMaterialsFromGLTF(materialCache, textureCache, scene,
                                            path);
  // 2. 处理所有子网格
      ...
      if (generateLODs) {
        ...
        for (size_t lodLevel = 1; lodLevel < lodLevels.size(); lodLevel++) {
          MeshData simplifiedMesh =
              SimplifyMesh(subMeshLodChain.baseSection, lodLevels[lodLevel],
                           subMeshLodChain.layout);
  ...
  CalculateBoundingBox(subMeshData, modelAsset->metadata.boundingBoxMin,
                       modelAsset->metadata.boundingBoxMax);

  // 3.2. 存储材质引用（MaterialAssetID）
  modelAsset->materialRefs = materialIDs;

  // 4. 创建ModelSourceData并构建MeshSectionLODChain
  std::shared_ptr<ModelSourceData> sourceData =
      CreateModelSourceData(modelAsset, subMeshData);
...
MeshData ModelLoader::ProcessMesh(const aiMesh *aiMesh, VertexLayout layout) {
  ...
  for (unsigned int i = 0; i < aiMesh->mNumVertices; i++) {
    // 位置坐标
    glm::vec3 position(aiMesh->mVertices[i].x, aiMesh->mVertices[i].y,
    ...
    if (aiMesh->HasNormals()) {
      glm::vec3 normal(aiMesh->mNormals[i].x, aiMesh->mNormals[i].y,
                       aiMesh->mNormals[i].z);
    ...
    if (aiMesh->mTextureCoords[0]) {
      glm::vec2 uv(aiMesh->mTextureCoords[0][i].x,
    ...
    if (aiMesh->HasTangentsAndBitangents()) {
      glm::vec3 tangent(aiMesh->mTangents[i].x, aiMesh->mTangents[i].y,
                        aiMesh->mTangents[i].z);
      memcpy(vertexPtr, &tangent, sizeof(glm::vec3));
      vertexPtr += sizeof(glm::vec3);

      glm::vec3 bitangent(aiMesh->mBitangents[i].x, aiMesh->mBitangents[i].y,
...
MeshData ModelLoader::SimplifyMesh(const MeshData &originalMesh,
                                   float targetRatio, VertexLayout layout) {
  ...
  size_t simplifiedIndexCount = meshopt_simplifySloppy(
      simplifiedIndices.data(), indices.data(), indexCount,
      reinterpret_cast<const float *>(originalMesh.vertexData.data()),
      vertexCount, layout.stride, targetIndexCount,
  ...
  size_t uniqueVertexCount =
      meshopt_optimizeVertexFetchRemap(remap.data(), simplifiedIndices.data(),
                                       simplifiedIndexCount, vertexCount);
  ...
  meshopt_remapVertexBuffer(simplifiedVertexData.data(),
                            originalMesh.vertexData.data(), vertexCount,
                            layout.stride, remap.data());

  // 重映射索引（相对于自己的顶点数据的相对偏移）
  meshopt_remapIndexBuffer(simplifiedIndices.data(), simplifiedIndices.data(),
                           simplifiedIndexCount, remap.data());
  ...
  for (size_t i = 0; i < simplifiedMesh.vertexData.size(); i += layout.stride) {
    glm::vec3 position;
    memcpy(&position, vPtr + i, sizeof(glm::vec3));
    simplifiedMesh.boundingBoxMin =
        glm::min(simplifiedMesh.boundingBoxMin, position);
    simplifiedMesh.boundingBoxMax =
        glm::max(simplifiedMesh.boundingBoxMax, position);
  }
...
void ModelLoader::CalculateBoundingBox(
    const std::vector<MeshDataLODChain> &subMeshes, glm::vec3 &outMin,
    glm::vec3 &outMax) {
  if (subMeshes.empty()) {
...
std::shared_ptr<ModelSourceData> ModelLoader::CreateModelSourceData(
    ...
    const std::vector<MeshDataLODChain> &subMeshData) {
  std::shared_ptr<ModelSourceData> sourceData =
      std::make_shared<ModelSourceData>();
  ...
  auto MergeMeshDataToSourceData =
      [&sourceData, &vertexOffset,
       &indexOffset](const MeshData &meshData) -> MeshSection {
    // 添加顶点数据
    ...
    for (auto &index : adjustedIndices) {
      // 修正索引值偏移，将单个 MeshData
      // 存储的相对偏移（相对于自己的顶点数据）， 修正为合并到 ModelSourceData
      // 后的绝对偏移（相对于合并后的顶点数据）
      index += vertexOffset;
    }

    // 执行合并操作
    sourceData->mergedIndices.insert(sourceData->mergedIndices.end(),
                                     adjustedIndices.begin(),
                                     adjustedIndices.end());

    // 创建基础 MeshSection
    MeshSection meshSection =
        MeshSection{vertexOffset,  // 顶点偏移（以顶点计数为单位）
                    indexOffset,   // 索引偏移（以索引计数为单位）
                    static_cast<uint32_t>(meshData.vertexData.size() /
                                          sourceData->layout.stride),
  ...
  for (const MeshDataLODChain &meshLODChain : subMeshData) {
    ...
    // 处理基础 LOD
    sectionLODChain.baseSection =
        MergeMeshDataToSourceData(meshLODChain.baseSection);

    // 处理其他 LOD 级别
    for (const MeshData &lodMeshData : meshLODChain.lodSections) {
      sectionLODChain.lodSections.push_back(
          MergeMeshDataToSourceData(lodMeshData));
    }
...
unsigned int ModelLoader::GetAssimpImportFlags(const std::string &extension,
                                               bool flipUVs) {
...
ModelAssetID ModelLoader::FindModelByPath(ModelCache &cache,
