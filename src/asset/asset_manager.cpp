...
TextureAssetID AssetManager::LoadTextureInternal(const std::string &path) {
  try {
    ...
    LOG_ERROR("[AssetManager] Texture load exception: " +
              std::string(e.what()));
    return TextureAssetID{};
  }
}
std::shared_ptr<TextureAsset> AssetManager::GetTexture(
    TextureAssetID id) const {
  std::lock_guard<std::mutex> lock(m_Mutex);
...
// ===================== 模型管理 =====================
ModelAssetID AssetManager::LoadGLTFModel(const std::string &path, bool flipUVs,
                                         bool generateLODs,
                                         const std::vector<float> &lodLevels) {
  std::lock_guard<std::mutex> lock(m_Mutex);
  return LoadModelInternal(path, flipUVs, generateLODs, lodLevels, true, false);
}
ModelAssetID AssetManager::LoadObjModel(const std::string &path, bool flipUVs,
                                        bool generateLODs,
                                        const std::vector<float> &lodLevels) {
  std::lock_guard<std::mutex> lock(m_Mutex);
  return LoadModelInternal(path, flipUVs, generateLODs, lodLevels, false, true);
}
ModelAssetID AssetManager::LoadModel(const std::string &path, bool flipUVs,
                                     bool generateLODs,
                                     const std::vector<float> &lodLevels) {
  std::lock_guard<std::mutex> lock(m_Mutex);
  return LoadModelInternal(path, flipUVs, generateLODs, lodLevels, false,
                           false);
}

ModelAssetID AssetManager::LoadModelInternal(
    const std::string &path, bool flipUVs, bool generateLODs,
    const std::vector<float> &lodLevels, bool isGLTF, bool isOBJ) {
  try {
    ModelAssetID modelID;

    if (isGLTF) {
      modelID = ModelLoader::LoadGLTFModel(m_ModelCache, m_MaterialCache,
                                           m_TextureCache, path, flipUVs,
                                           generateLODs, lodLevels);
    } else if (isOBJ) {
      modelID = ModelLoader::LoadObjModel(m_ModelCache, m_MaterialCache,
                                          m_TextureCache, path, flipUVs,
                                          generateLODs, lodLevels);
    } else {
      modelID =
          ModelLoader::LoadModel(m_ModelCache, m_MaterialCache, m_TextureCache,
...
// ===================== 材质管理 =====================
std::shared_ptr<MaterialAsset> AssetManager::GetMaterial(
    MaterialAssetID id) const {
  std::lock_guard<std::mutex> lock(m_Mutex);
...
size_t AssetManager::PurgeUnusedAssets() {
  ...
  if (totalPurged > 0) {
    LOG_INFO("[AssetManager] Purged " + std::to_string(totalPurged) +
             " unused assets: " + std::to_string(texturePurged) +
             " textures, " + std::to_string(modelPurged) + " models, " +
             std::to_string(materialPurged) + " materials");
  }
