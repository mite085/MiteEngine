...
class ModelLoader {
 public:
   ...
   */
  static ModelAssetID LoadGLTFModel(
      ModelCache &modelCache, MaterialCache &materialCache,
      TextureCache &textureCache, const std::string &path, bool flipUVs = false,
   ...
   * @param lodLevels LOD级别配置（每个级别的简化比例）
   */
  static ModelAssetID LoadObjModel(
      ModelCache &modelCache, MaterialCache &materialCache,
 ...
 private:
   ...
   * 核心加载实现
   */
  static ModelAssetID LoadModelInternal(
      ModelCache &modelCache, MaterialCache &materialCache,
  ...
  // 配置Assimp导入器标志（格式特化）
  static unsigned int GetAssimpImportFlags(const std::string &extension,
                                           bool flipUVs);

  // 通过路径查找已缓存的模型
  static ModelAssetID FindModelByPath(ModelCache &cache,
