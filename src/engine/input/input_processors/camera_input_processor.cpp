...
CameraInputProcessor::CameraInputProcessor(std::shared_ptr<Camera> camera)
  ...
  if (!s_Logger) {
    s_Logger =
        mite::LoggerSystem::CreateModuleLogger("Mite Input Processor: Camera");
...
bool CameraInputProcessor::handleMouseMove(MouseMoveEvent &e) {
  ...
  if (m_InputState.rotating) {
    // 右键旋转视角
    m_Camera->RotateWorld(-delta.x * m_RotationSpeed,
                          -delta.y * m_RotationSpeed);
    return true;  // 事件已处理
  } else if (m_InputState.panning) {
    // 中键平移视角
    m_Camera->Pan(delta.x * float(0.01) * m_MoveSpeed,
                  -delta.y * float(0.01) * m_MoveSpeed);
    return true;  // 事件已处理
  }
...
bool CameraInputProcessor::handleMouseButtonPressed(
...
bool CameraInputProcessor::handleMouseButtonReleased(
...
void CameraInputProcessor::UpdateMoveDirection() {
  ...
  if (m_InputState.keyStates[GLFW_KEY_S]) m_InputState.moveDirection.z -= 1.0f;
  if (m_InputState.keyStates[GLFW_KEY_A]) m_InputState.moveDirection.x -= 1.0f;
  if (m_InputState.keyStates[GLFW_KEY_D])
    // 与GetRightVector()相乘，R按键(向右)为正
    m_InputState.moveDirection.x += 1.0f;
  if (m_InputState.keyStates[GLFW_KEY_Q]) m_InputState.moveDirection.y -= 1.0f;
