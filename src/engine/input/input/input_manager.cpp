...
void InputManager::Init() {
  ...
  m_EventSubscriptions.SubscribeByCategoryImmediate(
      EventCategory::EVENT_CATEGORY_INPUT, BIND_DISPATCH_FN(ProcessEvent),
...
void InputManager::PushContext(std::shared_ptr<InputContext> context) {
  ...
  if (!m_ContextStack->IsEmpty()) {
    ...
    LOG_DEBUG("Pushing input context '{}' over '{}'", context->GetName(),
              current->GetName());
  } else {
...
void InputManager::PopContext() {
  ...
  if (!m_ContextStack->IsEmpty()) {
    ...
    LOG_DEBUG("Popped input context '{}', new current is '{}'",
              popped->GetName(), newCurrent->GetName());
  } else {
...
void InputManager::ProcessEvent(Event &e) { m_ContextStack->ProcessEvent(e); }
