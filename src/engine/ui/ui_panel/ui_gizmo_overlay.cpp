...
void GizmoOverlay::Render(OverlayContext &context) {
  ...
  if (context.viewportPos.x <= 0 || context.viewportPos.y <= 0) return;
  ...
  ImVec2 viewManipulatePosition =
      ImVec2(viewManipulateRight - m_ViewManipulateSize.x, viewManipulateTop);
  ...
  ImGuizmo::SetRect(context.viewportPos.x, context.viewportPos.y,
  ...
  if (context.isModelSelected) {
    ...
    ImGuizmo::ViewManipulate(
        glm::value_ptr(viewMatrix), optimalLength, viewManipulatePosition,
        {m_ViewManipulateSize.x, m_ViewManipulateSize.y}, 0x10101010);
    // 渲染Gizmo
    ImGuizmo::Manipulate(
        glm::value_ptr(viewMatrix), glm::value_ptr(context.cameraProjection),
        (ImGuizmo::OPERATION)m_CurrentOperation, (ImGuizmo::MODE)m_CurrentMode,
  ...
  // 若非选中状态，仅绘制ViewManipulate
    ...
    glm::mat4 tempModelMatrix(
        1.0f);  // 虽然ViewManipulate的float* matrix不参与计算，但不能为空

    // 绘制ViewManipulate，操控相机绕着距离为defaultLength的正前方的点旋转
    ImGuizmo::ViewManipulate(
        glm::value_ptr(viewMatrix), glm::value_ptr(context.cameraProjection),
        (ImGuizmo::OPERATION)m_CurrentOperation, (ImGuizmo::MODE)m_CurrentMode,
        glm::value_ptr(tempModelMatrix), defaultLength, viewManipulatePosition,
        {m_ViewManipulateSize.x, m_ViewManipulateSize.y}, 0x10101010);
  }
  // 临时变量反馈回Context
...
void GizmoOverlay::SetMode(int mode) { m_CurrentMode = mode; }
}  // namespace mite