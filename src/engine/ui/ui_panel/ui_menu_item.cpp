...
UIMenuItem::UIMenuItem(const std::string &label,
                       MenuItemType type,
                       bool selected,
...
UIMenuItem *UIMenuItemSubmenu::AddItem(const std::string &label, std::function<void()> callback)
{
  ...
  auto item = std::make_shared<UIMenuItem>(label, MenuItemType::ITEM, false, wrappedCallback);
  AddChild(item);
  return item.get();
}
...
UIMenuItem *UIMenuItemSubmenu::AddCheckbox(const std::string &label,
                                           ...
                                           std::function<void(bool)> callback)
{
  auto item = std::make_shared<UIMenuItem>(label, MenuItemType::CHECKBOX, checked, callback);
  AddChild(item);
  return item.get();
}
...
{
  auto separator = std::make_shared<UIMenuItem>("", MenuItemType::SEPARATOR, false, nullptr);
  AddChild(separator);
}
