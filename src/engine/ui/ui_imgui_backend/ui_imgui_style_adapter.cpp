...
ImGuiStyleAdapter::ImGuiStyleAdapter() {
  // 创建日志系统
  m_Logger =
      mite::LoggerSystem::CreateModuleLogger("Mite UI ImGui Style Adapter");
...
void ImGuiStyleAdapter::Initialize() {
  ...
  m_EventSubscriptions.SubscribeImmediate<StyleChangedEvent>(
      BIND_DISPATCH_FN(OnStyleChanged), EventPriority::Normal);
}
...
bool ImGuiStyleAdapter::ApplyUIStyle(const std::shared_ptr<UIStyle> uiStyle) {
  ...
  try {
    ...
    m_Logger->error("Failed to apply UI style {}: {}", uiStyle->GetName(),
                    e.what());
    return false;
  }
}

void ImGuiStyleAdapter::MapUIStyleToImGui(
    const std::shared_ptr<UIStyle> &uiStyle) {
  // 重置到默认样式
...
void ImGuiStyleAdapter::MapColorProperties(
    const std::shared_ptr<UIStyle> &uiStyle) {
  for (const auto &[propertyName, colorIndex] : m_ColorMappings) {
    if (uiStyle->HasProperty(propertyName)) {
      try {
        ...
        m_Logger->warn("Failed to map color property {}: {}", propertyName,
                       e.what());
      }
    }
  }
...
void ImGuiStyleAdapter::MapSizeProperties(
    const std::shared_ptr<UIStyle> &uiStyle) {
  for (const auto &[propertyName, setter] : m_SizeMappings) {
    if (uiStyle->HasProperty(propertyName)) {
      try {
        ...
        m_Logger->warn("Failed to map size property {}: {}", propertyName,
                       e.what());
      }
    }
  }
...
void ImGuiStyleAdapter::MapEnumProperties(
    const std::shared_ptr<UIStyle> &uiStyle) {
  ...
  for (const auto &[propertyName, setter] : m_EnumMappings) {
    if (uiStyle->HasProperty(propertyName)) {
      try {
        ...
        m_Logger->warn("Failed to map enum property {}: {}", propertyName,
                       e.what());
      }
    }
  }
...
void ImGuiStyleAdapter::MapBorderProperties(
    const std::shared_ptr<UIStyle> &uiStyle) {
  for (const auto &[propertyName, setter] : m_BorderMappings) {
    if (uiStyle->HasProperty(propertyName)) {
      try {
        ...
        m_Logger->warn("Failed to map border property {}: {}", propertyName,
                       e.what());
      }
    }
  }
...
void ImGuiStyleAdapter::MapSpacingProperties(
    const std::shared_ptr<UIStyle> &uiStyle) {
  for (const auto &[propertyName, setter] : m_SpacingMappings) {
    if (uiStyle->HasProperty(propertyName)) {
      try {
        ...
        m_Logger->warn("Failed to map spacing property {}: {}", propertyName,
                       e.what());
      }
    }
  }
...
void ImGuiStyleAdapter::SetImGuiColor(ImGuiCol colorIndex,
...
void ImGuiStyleAdapter::CreateDefaultStyleMappings() {
  ...
  m_ColorMappings = {
      ...
      {StyleProperties::COLOR_SCROLLBAR_GRAB_HOVER,
       ImGuiCol_ScrollbarGrabHovered},
      {StyleProperties::COLOR_SCROLLBAR_GRAB_ACTIVE,
      ...
      {StyleProperties::COLOR_TAB_UNFOCUSED_ACTIVE,
      ...
      {StyleProperties::COLOR_PLOT_HISTOGRAM_HOVER,
      ...
      {StyleProperties::COLOR_NAV_WINDOWING_HIGHLIGHT,
  ...
  m_SizeMappings = {
      {StyleProperties::SIZE_WINDOW_PADDING_X,
       [&](float v) { style.WindowPadding.x = v; }},
      {StyleProperties::SIZE_WINDOW_PADDING_Y,
       [&](float v) { style.WindowPadding.y = v; }},
      {StyleProperties::SIZE_WINDOW_ROUNDING,
       [&](float v) { style.WindowRounding = v; }},
      {StyleProperties::SIZE_WINDOW_MIN_SIZE_X,
       [&](float v) { style.WindowMinSize.x = v; }},
      {StyleProperties::SIZE_WINDOW_MIN_SIZE_Y,
       [&](float v) { style.WindowMinSize.y = v; }},
      {StyleProperties::SIZE_WINDOW_TITLE_ALIGN_X,
       [&](float v) { style.WindowTitleAlign.x = v; }},
      {StyleProperties::SIZE_WINDOW_TITLE_ALIGN_Y,
       [&](float v) { style.WindowTitleAlign.y = v; }},
      {StyleProperties::SIZE_CHILD_ROUNDING,
       [&](float v) { style.ChildRounding = v; }},
      {StyleProperties::SIZE_CHILD_BORDER_SIZE,
       [&](float v) { style.ChildBorderSize = v; }},
      {StyleProperties::SIZE_POPUP_ROUNDING,
       [&](float v) { style.PopupRounding = v; }},
      {StyleProperties::SIZE_POPUP_BORDER_SIZE,
       [&](float v) { style.PopupBorderSize = v; }},
      {StyleProperties::SIZE_FRAME_PADDING_X,
       [&](float v) { style.FramePadding.x = v; }},
      {StyleProperties::SIZE_FRAME_PADDING_Y,
       [&](float v) { style.FramePadding.y = v; }},
      {StyleProperties::SIZE_FRAME_ROUNDING,
       [&](float v) { style.FrameRounding = v; }},
      {StyleProperties::SIZE_FRAME_BORDER_SIZE,
       [&](float v) { style.FrameBorderSize = v; }},
      {StyleProperties::SIZE_ITEM_SPACING_X,
       [&](float v) { style.ItemSpacing.x = v; }},
      {StyleProperties::SIZE_ITEM_SPACING_Y,
       [&](float v) { style.ItemSpacing.y = v; }},
      {StyleProperties::SIZE_ITEM_INNER_SPACING_X,
       [&](float v) { style.ItemInnerSpacing.x = v; }},
      {StyleProperties::SIZE_ITEM_INNER_SPACING_Y,
       [&](float v) { style.ItemInnerSpacing.y = v; }},
      {StyleProperties::SIZE_CELL_PADDING_X,
       [&](float v) { style.CellPadding.x = v; }},
      {StyleProperties::SIZE_CELL_PADDING_Y,
       [&](float v) { style.CellPadding.y = v; }},
      ...
      {StyleProperties::SIZE_INDENT_SPACING,
       [&](float v) { style.IndentSpacing = v; }},
      {StyleProperties::SIZE_COLUMNS_MIN_SPACING,
       [&](float v) { style.ColumnsMinSpacing = v; }},
      {StyleProperties::SIZE_SCROLLBAR_SIZE,
       [&](float v) { style.ScrollbarSize = v; }},
      {StyleProperties::SIZE_SCROLLBAR_ROUNDING,
       [&](float v) { style.ScrollbarRounding = v; }},
      {StyleProperties::SIZE_GRAB_MIN_SIZE,
       [&](float v) { style.GrabMinSize = v; }},
      {StyleProperties::SIZE_GRAB_ROUNDING,
       [&](float v) { style.GrabRounding = v; }},
      {StyleProperties::SIZE_LOG_SLIDER_DEADZONE,
       [&](float v) { style.LogSliderDeadzone = v; }},
      {StyleProperties::SIZE_TAB_ROUNDING,
       [&](float v) { style.TabRounding = v; }},
      {StyleProperties::SIZE_TAB_BORDER_SIZE,
       [&](float v) { style.TabBorderSize = v; }},
      {StyleProperties::SIZE_TAB_MIN_WIDTH_FOR_CLOSE_BUTTON,
       [&](float v) { style.TabCloseButtonMinWidthUnselected = v; }},
      {StyleProperties::SIZE_BUTTON_TEXT_ALIGN_X,
       [&](float v) { style.ButtonTextAlign.x = v; }},
      {StyleProperties::SIZE_BUTTON_TEXT_ALIGN_Y,
       [&](float v) { style.ButtonTextAlign.y = v; }},
      ...
      {StyleProperties::SIZE_MOUSE_CURSOR_SCALE,
       [&](float v) { style.MouseCursorScale = v; }},
      {StyleProperties::SIZE_ANTI_ALIASED_LINES,
       [&](float v) { style.AntiAliasedLines = v; }},
      {StyleProperties::SIZE_ANTI_ALIASED_LINES_USE_TEX,
       [&](float v) { style.AntiAliasedLinesUseTex = v; }},
      {StyleProperties::SIZE_ANTI_ALIASED_FILL,
       [&](float v) { style.AntiAliasedFill = v; }},
  ...
  // 枚举映射
  m_EnumMappings = {
      {StyleProperties::LAYOUT_COLOR_BUTTON_POSITION,
  ...
  // 边框映射
  m_BorderMappings = {
      {StyleProperties::BORDER_WINDOW,
       [&](float v) { style.WindowBorderSize = v; }},
      {StyleProperties::BORDER_CHILD,
       [&](float v) { style.ChildBorderSize = v; }},
      {StyleProperties::BORDER_POPUP,
       [&](float v) { style.PopupBorderSize = v; }},
      {StyleProperties::BORDER_FRAME,
       [&](float v) { style.FrameBorderSize = v; }},
      {StyleProperties::BORDER_TAB, [&](float v) { style.TabBorderSize = v; }}};

  // 间距映射
  m_SpacingMappings = {{StyleProperties::SPACING_WINDOW_PADDING_X,
                        [&](float v) { style.WindowPadding.x = v; }},
                       {StyleProperties::SPACING_WINDOW_PADDING_Y,
                        [&](float v) { style.WindowPadding.y = v; }},
                       {StyleProperties::SPACING_FRAME_PADDING_X,
                        [&](float v) { style.FramePadding.x = v; }},
                       {StyleProperties::SPACING_FRAME_PADDING_Y,
                        [&](float v) { style.FramePadding.y = v; }},
                       {StyleProperties::SPACING_ITEM_SPACING_X,
                        [&](float v) { style.ItemSpacing.x = v; }},
                       {StyleProperties::SPACING_ITEM_SPACING_Y,
                        [&](float v) { style.ItemSpacing.y = v; }},
                       {StyleProperties::SPACING_ITEM_INNER_SPACING_X,
                        [&](float v) { style.ItemInnerSpacing.x = v; }},
                       {StyleProperties::SPACING_ITEM_INNER_SPACING_Y,
                        [&](float v) { style.ItemInnerSpacing.y = v; }},
                       {StyleProperties::SPACING_CELL_PADDING_X,
                        [&](float v) { style.CellPadding.x = v; }},
                       {StyleProperties::SPACING_CELL_PADDING_Y,
                        [&](float v) { style.CellPadding.y = v; }},
                       {StyleProperties::SPACING_TOUCH_EXTRA_PADDING_X,
                        [&](float v) { style.TouchExtraPadding.x = v; }},
                       {StyleProperties::SPACING_TOUCH_EXTRA_PADDING_Y,
                        [&](float v) { style.TouchExtraPadding.y = v; }},
                       {StyleProperties::SPACING_INDENT_SPACING,
                        [&](float v) { style.IndentSpacing = v; }},
                       {StyleProperties::SPACING_COLUMNS_MIN_SPACING,
                        [&](float v) { style.ColumnsMinSpacing = v; }}};
}

std::shared_ptr<UIStyle> ImGuiStyleAdapter::ExportToUIStyle(
  ...
  // 导出尺寸属性
  style->SetProperty(StyleProperties::SIZE_WINDOW_PADDING_X,
                     imguiStyle.WindowPadding.x);
  style->SetProperty(StyleProperties::SIZE_WINDOW_PADDING_Y,
                     imguiStyle.WindowPadding.y);
  style->SetProperty(StyleProperties::SIZE_FRAME_PADDING_X,
                     imguiStyle.FramePadding.x);
  style->SetProperty(StyleProperties::SIZE_FRAME_PADDING_Y,
                     imguiStyle.FramePadding.y);
  style->SetProperty(StyleProperties::SIZE_ITEM_SPACING_X,
                     imguiStyle.ItemSpacing.x);
  style->SetProperty(StyleProperties::SIZE_ITEM_SPACING_Y,
                     imguiStyle.ItemSpacing.y);
  style->SetProperty(StyleProperties::SIZE_WINDOW_ROUNDING,
                     imguiStyle.WindowRounding);
  style->SetProperty(StyleProperties::SIZE_FRAME_ROUNDING,
                     imguiStyle.FrameRounding);
  // 导出边框属性
  style->SetProperty(StyleProperties::BORDER_WINDOW,
...
ImGuiStyle &ImGuiStyleAdapter::GetImGuiStyle() { return ImGui::GetStyle(); }
