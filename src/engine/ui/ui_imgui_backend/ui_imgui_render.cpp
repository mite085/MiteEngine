...
void ImGuiUIRender::EndMenuBar() { ImGui::EndMenuBar(); }
bool ImGuiUIRender::RenderMenuItem(MenuItemProps &props) {
  ...
  if (props.isSeparator) {
    ...
    if (props.isCheckable) {
      // 复选框菜单项：传递selected的引用
      clicked = ImGui::MenuItem(menuItemText.c_str(), nullptr,
                                &props.selected,  // 传递引用
                                props.enabled);

      if (clicked && props.callback) {
        // 回调时props.selected已经被ImGui更新
        props.callback(props.selected);
      }
    } else {
      // 普通菜单项：不传递selected参数
      clicked = ImGui::MenuItem(menuItemText.c_str(), nullptr,
                                false,  // 固定为false，不显示勾选标记
                                props.enabled);
...
bool ImGuiUIRender::BeginPanel(PanelProps &props) {
  ...
  if (!props.movable) flags |= ImGuiWindowFlags_NoMove;
  if (!props.resizable) flags |= ImGuiWindowFlags_NoResize;
  if (!props.scrollable)
    flags |= ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse;
  if (props.collapsed) flags |= ImGuiWindowFlags_NoTitleBar;
  if (props.bringToFront) flags |= ImGuiWindowFlags_NoBringToFrontOnFocus;
  if (props.noBackground) flags |= ImGuiWindowFlags_NoBackground;
  if (props.hasMenuBar) flags |= ImGuiWindowFlags_MenuBar;
  if (!props.dockable) flags |= ImGuiWindowFlags_NoDocking;
...
bool ImGuiUIRender::BeginChild(ChildProps &props) {
  ...
  if (props.autoResizeX) childSize.x = 0;
  if (props.autoResizeY) childSize.y = 0;
  ...
  // 开始子窗口
  bool began =
      ImGui::BeginChild(childId.c_str(), childSize, props.border, flags);
...
bool ImGuiUIRender::IsPanelFocused() { return ImGui::IsWindowFocused(); }
bool ImGuiUIRender::IsPanelHovered() { return ImGui::IsWindowHovered(); }
...
bool ImGuiUIRender::RenderButton(const ButtonProps &props) {
  ...
  std::string displayText = GetTranslatedText(props);
  bool changed = ImGui::Button(displayText.c_str(),
...
bool ImGuiUIRender::RenderTextInput(TextInputProps &props) {
  ...
  bool changed = ImGui::InputTextWithHint(labelText.c_str(), hintText.c_str(),
                                          &props.text);
...
bool ImGuiUIRender::RenderTextArea(TextAreaProps &props) {
  ...
  std::string labelText = GetTranslatedText(props);
  bool changed =
      ImGui::InputTextMultiline(labelText.c_str(), &props.text,
                                ImVec2(static_cast<float>(props.size.x),
                                       static_cast<float>(props.size.y)));
...
bool ImGuiUIRender::RenderSliderInt(IntEditProps &props) {
  ...
  std::string labelText = GetTranslatedText(props);
  bool changed =
      ImGui::SliderInt(labelText.c_str(), &props.value, props.minValue,
                       props.maxValue, props.format.c_str());
  ImGui::PopID();
  return changed;
}
...
bool ImGuiUIRender::RenderSliderFloat(FloatEditProps &props) {
  ...
  std::string labelText = GetTranslatedText(props);
  bool changed =
      ImGui::SliderFloat(labelText.c_str(), &props.value, props.minValue,
                         props.maxValue, props.format.c_str());
  ImGui::PopID();
  return changed;
}
...
bool ImGuiUIRender::RenderSliderFloat2(Float2EditProps &props) {
  ...
  std::string labelText = GetTranslatedText(props);
  bool changed =
      ImGui::SliderFloat2(labelText.c_str(), glm::value_ptr(props.value),
                          props.minValue, props.maxValue, props.format.c_str());
  ImGui::PopID();
  return changed;
}
...
bool ImGuiUIRender::RenderSliderFloat3(Float3EditProps &props) {
  ...
  std::string labelText = GetTranslatedText(props);
  bool changed =
      ImGui::SliderFloat3(labelText.c_str(), glm::value_ptr(props.value),
                          props.minValue, props.maxValue, props.format.c_str());
  ImGui::PopID();
  return changed;
}
...
bool ImGuiUIRender::RenderSliderFloat4(Float4EditProps &props) {
  ...
  std::string labelText = GetTranslatedText(props);
  bool changed =
      ImGui::SliderFloat4(labelText.c_str(), glm::value_ptr(props.value),
                          props.minValue, props.maxValue, props.format.c_str());
  ImGui::PopID();
  return changed;
}
...
bool ImGuiUIRender::RenderDragInt(IntEditProps &props) {
  ...
  std::string labelText = GetTranslatedText(props);
  bool changed =
      ImGui::DragInt(labelText.c_str(), &props.value, props.dragSpeed,
                     props.minValue, props.maxValue, props.format.c_str());
  ImGui::PopID();
  return changed;
}
...
bool ImGuiUIRender::RenderDragFloat(FloatEditProps &props) {
  ...
  std::string labelText = GetTranslatedText(props);
  bool changed =
      ImGui::DragFloat(labelText.c_str(), &props.value, props.dragSpeed,
                       props.minValue, props.maxValue, props.format.c_str());
  ImGui::PopID();
  return changed;
}
...
bool ImGuiUIRender::RenderDragFloat2(Float2EditProps &props) {
  ...
  bool changed = ImGui::DragFloat2(
      labelText.c_str(), glm::value_ptr(props.value), props.dragSpeed,
      props.minValue, props.maxValue, props.format.c_str());
  ImGui::PopID();
  return changed;
}
...
bool ImGuiUIRender::RenderColorEdit(ColorEditProps &props) {
  ...
  bool changed =
      ImGui::ColorEdit4(labelText.c_str(), glm::value_ptr(props.color), flags);
...
void ImGuiUIRender::RenderTreeNode(
  ...
  if (props.isSelect) flags |= ImGuiTreeNodeFlags_Selected;
...
void ImGuiUIRender::RenderTreeVoid(
    const std::function<void(void *)> &dragDropTargetContent) {
...
bool ImGuiUIRender::RenderPopup(PopupProps &props,
                                const std::function<void()> &renderContent) {
  if (!props.visible) return false;
  ImGui::PushID(&props);
...
void ImGuiUIRender::RenderTable(TableProps &props,
                                const std::function<void()> &renderContent) {
  if (!props.visible) return;
  ImGui::PushID(&props);
  std::string labelText = GetTranslatedText(props);
  ...
  // 基础特性设置
  if (props.resizable) flags |= ImGuiTableFlags_Resizable;
  if (props.reorderable) flags |= ImGuiTableFlags_Reorderable;
  if (props.hideable) flags |= ImGuiTableFlags_Hideable;
  if (props.sortable) flags |= ImGuiTableFlags_Sortable;
  // 装饰选项设置
  if (props.rowBg) flags |= ImGuiTableFlags_RowBg;
  if (props.borders)
    flags |= ImGuiTableFlags_Borders;
  else {
    if (props.bordersInnerH) flags |= ImGuiTableFlags_BordersInnerH;
    if (props.bordersInnerV) flags |= ImGuiTableFlags_BordersInnerV;
    if (props.bordersOuterH) flags |= ImGuiTableFlags_BordersOuterH;
    if (props.bordersOuterV) flags |= ImGuiTableFlags_BordersOuterV;
  }
  // 尺寸策略设置
  ...
  // 尺寸额外选项设置
  if (props.noHostExtendX) flags |= ImGuiTableFlags_NoHostExtendX;
  if (props.noHostExtendY) flags |= ImGuiTableFlags_NoHostExtendY;
  if (props.preciseWidths) flags |= ImGuiTableFlags_PreciseWidths;
  // 滚动选项设置
  if (props.scrollX) flags |= ImGuiTableFlags_ScrollX;
  if (props.scrollY) flags |= ImGuiTableFlags_ScrollY;
  // 填充选项设置
  if (props.padOuterX) flags |= ImGuiTableFlags_PadOuterX;
  if (props.noPadInnerX) flags |= ImGuiTableFlags_NoPadInnerX;
...
void ImGuiUIRender::TableNextColume() { ImGui::TableNextColumn(); }

// ==================== 布局控件渲染实现 ====================

void ImGuiUIRender::RenderSeparator() { ImGui::Separator(); }

void ImGuiUIRender::RenderSpacer(const SpacerProps &props) {
  if (!props.visible) return;
  ImGui::PushID(&props);

  ImGui::Dummy(ImVec2(static_cast<float>(props.size.x),
                      static_cast<float>(props.size.y)));
...
void ImGuiUIRender::SetNewLine() { ImGui::NewLine(); }
...
void ImGuiUIRender::EndDisabled() { ImGui::EndDisabled(); }
...
void ImGuiUIRender::OpenFileDialog(
  ...
  // 打开对话框
  ImGuiFileDialog::Instance()->OpenDialog(dialogKey, GetTranslatedText(title),
...
std::string ImGuiUIRender::GetTranslatedText(
...
std::string ImGuiUIRender::GetTranslatedHeader(const TableProps &props,
  ...
  if (columnIndex < props.headerTranslationKeys.size() &&
    ...
    return UILocalization::Get().Translate(
        props.headerTranslationKeys[columnIndex].c_str());
  }
  return "";
}
