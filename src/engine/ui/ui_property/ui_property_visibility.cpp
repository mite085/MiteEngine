...
namespace mite {
const EnumComboBoxList<uint32_t, 2> PropertyTable<VisibilityComponent>::m_VisibilityMaskList =
    EnumComboBoxList(std::array{std::pair{VisibilityMask::NONE, "editor.visibility_mask_none"},
...
void PropertyTable<VisibilityComponent>::Render(UIRender &render)
{
  ...
  RenderLabelItemRow(render, "editor.visibility_mask", [&]() {
    ...
    if (render.RenderCombobox(m_VisibilityMaskProps)) {
      m_Component.SetVisibilityMask(
          m_VisibilityMaskList.GetEnumType(m_VisibilityMaskProps.selectedIndex)); // 使用选择后的Index执行Set逻辑
    }
  });
}
}  // namespace mite