...
// 设置标志位
inline SubscriptionFlags SetFlag(SubscriptionFlags flags,
                                 SubscriptionFlags setFlag) {
  return static_cast<SubscriptionFlags>(static_cast<uint8_t>(flags) |
                                        static_cast<uint8_t>(setFlag));
}

// 清除标志位
inline SubscriptionFlags ClearFlag(SubscriptionFlags flags,
                                   SubscriptionFlags clearFlag) {
  return static_cast<SubscriptionFlags>(static_cast<uint8_t>(flags) &
                                        ~static_cast<uint8_t>(clearFlag));
}

// 切换标志位
inline SubscriptionFlags ToggleFlag(SubscriptionFlags flags,
                                    SubscriptionFlags toggleFlag) {
  return static_cast<SubscriptionFlags>(static_cast<uint8_t>(flags) ^
...
inline bool IsDeferredAsync(SubscriptionFlags flags) {
  return HasFlag(flags, SubscriptionFlags::Async) &&
         HasFlag(flags, SubscriptionFlags::Deferred);
}
// 检查是否包含线程安全标志
...
inline SubscriptionFlags operator|(SubscriptionFlags lhs,
                                   SubscriptionFlags rhs) {
  return SubscriptionFlagUtil::SetFlag(lhs, rhs);
}
inline SubscriptionFlags operator&(SubscriptionFlags lhs,
                                   SubscriptionFlags rhs) {
  return static_cast<SubscriptionFlags>(static_cast<uint8_t>(lhs) &
