...
namespace mite {
ShadowInstance::ShadowInstance() { LOG_DEBUG("ShadowInstance allocated"); }
...
bool ShadowInstance::InitializeUBO() {
  ...
  try {
    // 创建阴影UBO对象
    m_ShadowUBO = std::make_shared<ShaderUBO>(
        sizeof(ShadowUniformBuffer),
        BindingPointManager::Get().GetShadowUBOBinding(), GL_DYNAMIC_DRAW);
...
bool ShadowInstance::UpdateUBO(
  ...
  try {
    ...
    for (const auto &light : lights) {
      ...
      switch (type) {
        case LightType::DIRECTIONAL: {
          ...
          if (ProcessDirectionalLight(light, transformIt->second,
                                      directionalCount, cameraInstance)) {
            directionalCount++;
          }
          break;
        }
        ...
        case LightType::POINT: {
          if (pointCount >= MAX_POINT_LIGHTS) {
            LOG_WARN("Exceeded maximum point lights for shadows: {}",
                     MAX_POINT_LIGHTS);
            continue;
          }

          if (ProcessPointLight(light, transformIt->second, pointCount,
                                cameraInstance)) {
            pointCount++;
          }
          break;
        ...
        case LightType::SPOT: {
          if (spotCount >= MAX_SPOT_LIGHTS) {
            LOG_WARN("Exceeded maximum spot lights for shadows: {}",
                     MAX_SPOT_LIGHTS);
            continue;
          }

          if (ProcessSpotLight(light, transformIt->second, spotCount,
                               cameraInstance)) {
            spotCount++;
          }
          break;
        ...
        default:
          // 其他类型的光源不支持阴影，静默跳过
          LOG_TRACE("Light type {} does not support shadows",
    ...
    // 更新阴影配置
    m_ShadowData.shadowConfig =
        glm::ivec4(directionalCount, pointCount, spotCount, 0);
    ...
    // 更新UBO数据
    bool success =
        m_ShadowUBO->UpdateData(&m_ShadowData, sizeof(ShadowUniformBuffer));
    if (success) {
      LOG_TRACE(
          "ShadowInstance UBO updated - Directional: {}, Point: {}, Spot: {}",
          directionalCount, pointCount, spotCount);
    } else {
...
bool ShadowInstance::ProcessDirectionalLight(
    std::shared_ptr<Light> light, const Transform &lightTransform,
    uint32_t lightIndex, std::shared_ptr<CameraInstance> cameraInstance) {
  // 类型转换
  ...
  ShadowMapData shadowData = light->PrepareShadowData(
      lightIndex, lightTransform, cameraInstance->GetCameraTransform(),
      cameraInstance->GetProjectionMatrix());
  if (!shadowData.enabled || !shadowData.isValid) {
    return false;
  }
  // 设置阴影索引
  m_ShadowData.directionalShadowIndices[lightIndex] =
      glm::ivec4(shadowData.shadowMapIndex, 0, 0, 0);

  // 设置级联分割距离
  for (uint32_t cascadeIdx = 0;
       cascadeIdx < shadowData.specific.directional.cascadeCount &&
       cascadeIdx < MAX_CASCADES;
       cascadeIdx++) {
...
bool ShadowInstance::ProcessPointLight(
    std::shared_ptr<Light> light, const Transform &lightTransform,
    uint32_t lightIndex, std::shared_ptr<CameraInstance> cameraInstance) {
  // 类型转换
  ...
  // 设置阴影索引
  m_ShadowData.pointShadowIndices[lightIndex] =
      glm::ivec4(shadowData.shadowMapIndex, 0, 0, 0);
...
bool ShadowInstance::ProcessSpotLight(
    std::shared_ptr<Light> light, const Transform &lightTransform,
    uint32_t lightIndex, std::shared_ptr<CameraInstance> cameraInstance) {
  // 类型转换
  ...
  // 设置阴影索引
  m_ShadowData.spotShadowIndices[lightIndex] =
      glm::ivec4(shadowData.shadowMapIndex, 0, 0, 0);

  // 设置聚光灯阴影矩阵
  m_ShadowData.spotLightMatrices[lightIndex] =
      shadowData.specific.spot.projectionMatrix *
