...
void Light::SetColor(const glm::vec3 &color) { m_Properties.color = color; }
const glm::vec3 &Light::GetColor() const { return m_Properties.color; }

void Light::SetIntensity(float intensity) {
  m_Properties.intensity = intensity;
}
float Light::GetIntensity() const { return m_Properties.intensity; }

void Light::SetEnabled(bool enabled) { m_Properties.enabled = enabled; }
bool Light::IsEnabled() const { return m_Properties.enabled; }
LightProperties &Light::GetProperties() { return m_Properties; }
const LightProperties &Light::GetProperties() const { return m_Properties; }
void Light::SetProperties(const LightProperties &props) {
  m_Properties = props;
}
void Light::SetShadowMap(ShadowMapPtr shadow) { m_ShadowMap = shadow; }
ShadowMapPtr Light::GetShadowMap() const { return m_ShadowMap; }
...
ShadowMapData Light::PrepareShadowData(const uint32_t lightIndex,
  ...
  if (m_ShadowMap && m_Properties.enabled) {
    return m_ShadowMap->PrepareShadowData(lightIndex, worldTransform,
                                          cameraView, cameraProj);
  }
  return ShadowMapData();
}
bool Light::Validate() const { return ValidateBaseParameters(); }
