...
PointShadowMap::PointShadowMap(const ShadowMapData &data)
  ...
  LOG_TRACE("PointShadowMap created - near: {}, far: {}",
            m_Data.specific.point.nearPlane, m_Data.specific.point.farPlane);
}

ShadowMapData PointShadowMap::PrepareShadowData(
...
glm::mat4 PointShadowMap::GetShadowMatrix(size_t index) const {
  if (index >= 6) {
    LOG_ERROR("Invalid shadow matrix index for PointShadowMap: {} (max: 5)",
              index);
...
bool PointShadowMap::NeedsUpdate() const { return m_NeedsUpdate; }
...
void PointShadowMap::SetShadowRange(float nearPlane, float farPlane) {
  if (nearPlane <= 0.0f || farPlane <= nearPlane) {
    LOG_ERROR("Invalid shadow range for PointShadowMap: near={}, far={}",
              nearPlane, farPlane);
    return;
  }
  ...
  LOG_TRACE("PointShadowMap range updated - near: {}, far: {}", nearPlane,
            farPlane);
}
...
void PointShadowMap::CalculateCubeFaceMatrices(const glm::vec3 &lightPosition) {
  ...
  LOG_TRACE("Near: {}, Far: {}", m_Data.specific.point.nearPlane,
            m_Data.specific.point.farPlane);
  LOG_TRACE("Projection Matrix:");
  for (int row = 0; row < 4; ++row) {
    LOG_TRACE("  [{:8.4f}, {:8.4f}, {:8.4f}, {:8.4f}]", shadowProj[0][row],
              shadowProj[1][row], shadowProj[2][row], shadowProj[3][row]);
  }
  // 计算每个面的视图投影矩阵
  for (size_t i = 0; i < 6; ++i) {
    glm::mat4 shadowView =
        glm::lookAt(lightPosition, lightPosition + targets[i], ups[i]);
    ...
    LOG_TRACE("Target: ({}, {}, {}), Up: ({}, {}, {})", targets[i].x,
              targets[i].y, targets[i].z, ups[i].x, ups[i].y, ups[i].z);

    LOG_TRACE("View Matrix:");
    for (int row = 0; row < 4; ++row) {
      LOG_TRACE("  [{:8.4f}, {:8.4f}, {:8.4f}, {:8.4f}]", shadowView[0][row],
                shadowView[1][row], shadowView[2][row], shadowView[3][row]);
    }

    LOG_TRACE("ViewProjection Matrix:");
    for (int row = 0; row < 4; ++row) {
      LOG_TRACE("  [{:8.4f}, {:8.4f}, {:8.4f}, {:8.4f}]",
                shadowViewProj[0][row], shadowViewProj[1][row],
                shadowViewProj[2][row], shadowViewProj[3][row]);
    }
...
bool PointShadowMap::HasLightMoved(const glm::vec3 &newPosition) const {
  ...
  if (moved) {
    LOG_TRACE(
        "PointShadowMap light moved - distance: {}, from ({}, {}, {}) to ({}, "
        "{}, {})",
        distance, m_LastLightPosition.x, m_LastLightPosition.y,
        m_LastLightPosition.z, newPosition.x, newPosition.y, newPosition.z);
  }
