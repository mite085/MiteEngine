      ...
      m_AspectRatio(1.0f)
{
  ...
  LOG_TRACE("SpotShadowMap created - near: {}, far: {}, fov: {}",
            m_Data.specific.spot.nearPlane,
                                               ...
                                               const glm::mat4 &cameraProj)
{
  ...
  // 检查是否需要更新阴影矩阵
  if (m_NeedsUpdate || HasLightTransformChanged(lightPosition, lightDirection)) {
    ...
    LOG_TRACE("SpotShadowMap matrix updated for light at ({}, {}, {}), direction ({}, {}, {})",
              lightPosition.x,
              lightPosition.y,
              lightPosition.z,
              lightDirection.x,
...
{
  if (index != 0) {
    LOG_ERROR("Invalid shadow matrix index for SpotShadowMap: {} (must be 0)", index);
  ...
  return m_Data.specific.spot.projectionMatrix * m_Data.specific.spot.viewMatrix;
}

bool SpotShadowMap::NeedsUpdate() const
{
...
std::string SpotShadowMap::GetShadowTypeName() const
{
...
void SpotShadowMap::SetShadowRange(float nearPlane, float farPlane)
{
  if (nearPlane <= 0.0f || farPlane <= nearPlane) {
    LOG_ERROR("Invalid shadow range for SpotShadowMap: near={}, far={}", nearPlane, farPlane);
    return;
  }
  ...
  LOG_TRACE("SpotShadowMap range updated - near: {}, far: {}", nearPlane, farPlane);
}

void SpotShadowMap::SetPerspectiveParams(float fov, float aspectRatio)
{
  if (fov <= 0.0f || fov >= 180.0f) {
    LOG_ERROR("Invalid FOV for SpotShadowMap: {} (must be between 0 and 180 degrees)", fov);
    return;
  }
  ...
  LOG_TRACE("SpotShadowMap perspective params updated - fov: {}, aspect: {}", fov, aspectRatio);
}
...
float SpotShadowMap::GetFov() const
{
  return m_Fov;
}

float SpotShadowMap::GetAspectRatio() const
{
                                             ...
                                             const glm::vec3 &lightDirection)
{
  ...
  m_Data.specific.spot.viewMatrix = glm::lookAt(
      lightPosition, target, glm::vec3(0.0f, 1.0f, 0.0f));

  // 创建聚光灯的透视投影矩阵
  m_Data.specific.spot.projectionMatrix = glm::perspective(glm::radians(m_Fov),
                                                           m_AspectRatio,
                                                           m_Data.specific.spot.nearPlane,
                                                           m_Data.specific.spot.farPlane);

  LOG_TRACE(
      "SpotShadowMap matrix calculated - position: ({}, {}, {}), direction: ({}, {}, {}), fov: {}",
      lightPosition.x,
      lightPosition.y,
      lightPosition.z,
      lightDirection.x,
...
bool SpotShadowMap::HasLightTransformChanged(const glm::vec3 &newPosition,
                                             const glm::vec3 &newDirection) const
{
  ...
  if (positionChanged || directionChanged) {
    LOG_TRACE(
        "SpotShadowMap light transform changed - position moved: {} m, direction rotated: {} deg",
        positionDistance,
        directionAngle);
  }
