...
class OpenGLShader {
 public:
  ...
  // */
  // void SetUniformBlockBinding(const std::string &uniformBlockName, uint32_t
  // bindingPoint);
  ///**
  ...
  // * @param bindingPoint 绑定点索引
  // */
  // void SetShaderStorageBlockBinding(const std::string &storageBlockName,
  ...
  // * @return 块索引，如果不存在返回GL_INVALID_INDEX
  // */
  // uint32_t GetShaderStorageBlockIndex(const std::string &storageBlockName)
  // const;
  ...
  // ---- 工具函数 ----
  ShaderGPUHandle GetHandle() const { return m_Handle; }
 ...
 private:
  ...
  std::vector<uint32_t> CompileFileToSPIRV(const std::string &filename,
                                           shaderc_shader_kind kind);

  // ---- SPIR-V到OpenGL转换 ----
  uint32_t CompileSPIRVToGLShader(const std::vector<uint32_t> &spirv,
                                  uint32_t type);
  ...
  // mutable std::unordered_map<std::string, uint32_t> m_UniformBlockCache;  //
  // Uniform区块缓存 mutable std::unordered_map<std::string, uint32_t>
  // m_StorageBlockCache;  // Storage区块缓存
};
