...
BoundingVolumeIntersection::Intersects(const BoundingVolumeAABB &aabb,
  ...
  if (aabbInsideSphere) return IntersectionType::Covered;
...
BoundingVolumeIntersection::Intersects(const BoundingVolumeAABB &aabb,
  ...
  if (distance < -r) return IntersectionType::Inside;
  return IntersectionType::Intersect;
}

BoundingVolumeIntersection::IntersectionType
BoundingVolumeIntersection::Intersects(const BoundingVolumeAABB &aabb1,
                                       const BoundingVolumeAABB &aabb2) {
  if (!aabb1.Intersects(aabb2)) return IntersectionType::Outside;
  if (aabb1.Contains(aabb2)) return IntersectionType::Inside;
  if (aabb2.Contains(aabb1)) return IntersectionType::Covered;
  return IntersectionType::Intersect;
}
...
BoundingVolumeIntersection::IntersectionType
BoundingVolumeIntersection::Intersects(const BoundingVolumeSphere &sphere,
                                       const BoundingVolumeAABB &aabb) {
  // 利用对称性，但需要交换Inside和Covered
  IntersectionType result = Intersects(aabb, sphere);
  if (result == IntersectionType::Inside) return IntersectionType::Covered;
  if (result == IntersectionType::Covered) return IntersectionType::Inside;
  return result;
}
...
BoundingVolumeIntersection::Intersects(const BoundingVolumeSphere &sphere,
  ...
  // 将点转换到OBB局部空间
  glm::vec3 localPoint =
      glm::transpose(obb.orientation) * (sphere.center - obb.center);
  ...
  // 检查球是否完全在OBB内部
  bool sphereInsideOBB =
      (std::abs(localPoint.x) <= obb.extents.x - sphere.radius) &&
      (std::abs(localPoint.y) <= obb.extents.y - sphere.radius) &&
      (std::abs(localPoint.z) <= obb.extents.z - sphere.radius);

  if (sphereInsideOBB) return IntersectionType::Inside;
  ...
  if (obbInsideSphere) return IntersectionType::Covered;
  return IntersectionType::Intersect;
}
...
BoundingVolumeIntersection::Intersects(const BoundingVolumeSphere &sphere,
  ...
  if (distance > sphere.radius) return IntersectionType::Outside;
  if (distance < -sphere.radius) return IntersectionType::Inside;
  return IntersectionType::Intersect;
}

BoundingVolumeIntersection::IntersectionType
BoundingVolumeIntersection::Intersects(const BoundingVolumeSphere &sphere1,
  ...
  if (centerDist > radiusSum) return IntersectionType::Outside;
...
BoundingVolumeIntersection::IntersectionType
BoundingVolumeIntersection::Intersects(const BoundingVolumeOBB &obb,
                                       const BoundingVolumeAABB &aabb) {
  // 利用对称性，但需要交换Inside和Covered
...
BoundingVolumeIntersection::Intersects(const BoundingVolumeOBB &obb,
  ...
  // 计算OBB在平面法线方向上的投影范围
  float r =
      obb.extents.x * std::abs(glm::dot(plane.normal, obb.orientation[0])) +
...
BoundingVolumeIntersection::IntersectionType
BoundingVolumeIntersection::Intersects(const BoundingVolumeOBB &obb1,
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    if (distance > ra + rb) return IntersectionType::Outside;
  }
  ...
  for (int i = 0; i < 8; ++i) {
    ...
    vertex.x +=
        orientation2InLocal1[0].x * obb2.extents.x * (i & 1 ? 1.0f : -1.0f);
    vertex.y +=
        orientation2InLocal1[1].y * obb2.extents.y * (i & 2 ? 1.0f : -1.0f);
    vertex.z +=
        orientation2InLocal1[2].z * obb2.extents.z * (i & 4 ? 1.0f : -1.0f);

    if (std::abs(vertex.x) > obb1.extents.x ||
        std::abs(vertex.y) > obb1.extents.y ||
  ...
  if (obb2InsideObb1) return IntersectionType::Inside;
  ...
  for (int i = 0; i < 8; ++i) {
    ...
    vertex.x +=
        orientation1InLocal2[0].x * obb1.extents.x * (i & 1 ? 1.0f : -1.0f);
    vertex.y +=
        orientation1InLocal2[1].y * obb1.extents.y * (i & 2 ? 1.0f : -1.0f);
    vertex.z +=
        orientation1InLocal2[2].z * obb1.extents.z * (i & 4 ? 1.0f : -1.0f);

    if (std::abs(vertex.x) > obb2.extents.x ||
        std::abs(vertex.y) > obb2.extents.y ||
  ...
  if (obb1InsideObb2) return IntersectionType::Covered;
  return IntersectionType::Intersect;
}
...
BoundingVolumeIntersection::IntersectionType
BoundingVolumeIntersection::Intersects(const BoundingVolumePlane &plane,
                                       const BoundingVolumeAABB &aabb) {
