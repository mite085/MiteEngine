...
float Camera::GetAspectRatio() const { return m_Aspect; }
...
CameraUniformBuffer Camera::FillUBOData(const glm::mat4 &viewMatrix) const {
  ...
  // 判断投影类型 (1 = 透视, 0 = 正交)
  uboData.projectionType =
      (m_ProjectionType == CameraProjectionType::PERSPECTIVE) ? 1 : 0;
...
void Camera::UpdateProjection() const {
  // 透视相机
  if (m_ProjectionType == CameraProjectionType::PERSPECTIVE) {
    // glm::perspective第一个参数T fovy
    // 是垂直方向的视野角度，固定了垂直方向的视野范围
    ...
    if (m_Aspect >= 1.0f) {
      ...
      // 宽高比 < 1，使用水平FOV（短边是宽度）
      // 将垂直FOV转换为水平FOV：fov_horizontal = 2 * atan(tan(fov_vertical/2) *
      // aspect)
      float horizontalFOV =
          2.0f * glm::atan(glm::tan(glm::radians(m_FOV) * 0.5f) / m_Aspect);
      m_ProjectionMatrix =
          glm::perspective(horizontalFOV, m_Aspect, m_Near, m_Far);
    }
  }
  // 正交相机
    ...
    m_ProjectionMatrix = glm::ortho(-width / 2, width / 2, -height / 2,
                                    height / 2, m_Near, m_Far);
  } else {
