...
bool MeshInstance::InitializeUBO() {
  ...
  try {
    ...
    m_ModelUBO = std::make_shared<ShaderUBO>(
        ...
        BindingPointManager::Get().GetModelUBOBinding(), GL_DYNAMIC_DRAW);
...
//{
//   // 设置着色器绑定
//   m_ModelUBO->SetupShaderBinding(shader,
...
void MeshInstance::UpdateUBO(const Transform &worldTransform) {
  ...
  try {
    ...
    uboData.model =
        worldTransform
            .GetLocalMatrix();  // world transform的local matrix即为world matrix
    uboData.normalMatrix =
        glm::transpose(glm::inverse(glm::mat3(uboData.model)));
...
std::pair<glm::vec3, glm::vec3> MeshInstance::GetWorldBoundingBox() const {
  ...
  glm::vec3 worldMin =
      m_WorldTransform.GetLocalMatrix() * glm::vec4(localBBox.first, 1.0f);
  glm::vec3 worldMax =
      m_WorldTransform.GetLocalMatrix() * glm::vec4(localBBox.second, 1.0f);
