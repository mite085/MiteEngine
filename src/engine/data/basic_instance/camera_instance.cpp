...
bool CameraInstance::InitializeUBO() {
  ...
  try {
    ...
    m_CameraUBO = std::make_shared<ShaderUBO>(
        ...
        BindingPointManager::Get().GetCameraUBOBinding(), GL_DYNAMIC_DRAW);
...
//{
//   // 设置着色器绑定
//   m_CameraUBO->SetupShaderBinding(shader,
...
bool CameraInstance::UpdateUBO(const Transform cameraTransform) {
  ...
  try {
    ...
    CameraUniformBuffer uboData =
        m_Camera->FillUBOData(cameraTransform.GetViewMatrix());

    // 更新UBO数据
    bool success =
        m_CameraUBO->UpdateData(&uboData, sizeof(CameraUniformBuffer));
