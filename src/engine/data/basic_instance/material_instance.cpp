...
void MaterialInstance::InitializeUBO() {
  ...
  m_UBO = std::make_shared<ShaderUBO>(
      ...
      BindingPointManager::Get().GetMaterialUBOBinding(), GL_DYNAMIC_DRAW);
  m_UBO->Initialize();

  LOG_INFO("Material UBO initialized for '{}'", m_Name);
}

// void MaterialInstance::SetupShaderBinding(std::shared_ptr<OpenGLShader>
// shader)
//{
//   // 设置着色器绑定
...
MaterialType MaterialInstance::GetMaterialType() const {
  return static_cast<MaterialType>(
      static_cast<int>(m_MaterialData.materialInfo.x));
}
// ---- 基础PBR参数 ----
...
glm::vec3 MaterialInstance::GetShadowTint() const {
  return glm::vec3(m_MaterialData.nprColors.x, m_MaterialData.nprColors.y,
                   m_MaterialData.nprColors.z);
}
...
void MaterialInstance::SetupTextureBinding(TextureGPUSlot texture,
  ...
  // 移除已存在的相同绑定点纹理
  if (m_Textures.find(type) != m_Textures.end()) m_Textures.erase(type);
...
TextureGPUSlot MaterialInstance::GetMetallicRoughnessTexture() const {
  if (m_Textures.find(ExternalTextureType::MetallicRoughness) !=
      m_Textures.end()) {
...
// ===================== 绑定相关 =====================
size_t MaterialInstance::BindTexturesOnly(
    ExternalTextureBindFunc textureBindFunc) const {
  // 注意:
  // 为避免layout(binding = 18) uniform sampler2D
  // u_BaseColorTexture;等纹理槽位悬空
  ...
  auto bindOperator = [&](ExternalTextureType type) {
    if (m_Textures.find(type) != m_Textures.end())
      textureBindFunc(type, m_Textures.at(type).gpuHandle,
                      m_Textures.at(type).target);
    else
...
std::string MaterialInstance::GetName() const { return m_Name; }
