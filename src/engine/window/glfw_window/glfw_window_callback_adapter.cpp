...
GLFWWindowCallbackAdapter::GLFWWindowCallbackAdapter() : CallbackAdapter() {
  // 首次创建时初始化日志系统
  if (!s_Logger) {
    s_Logger = mite::LoggerSystem::CreateModuleLogger(
        "Mite GLFW Window Callback Adapter");
...
void GLFWWindowCallbackAdapter::ErrorCallback(int error,
                                              const char *description) {
  s_Logger->error("GLFW Error ({}): {}", error, description);
}
void GLFWWindowCallbackAdapter::HandleWindowClose(
    [[maybe_unused]] GLFWwindow *window) {
  EventBus::Publish<WindowCloseEvent>();
  s_Logger->info("Window close requested");
}
void GLFWWindowCallbackAdapter::HandleWindowResize(
...
void GLFWWindowCallbackAdapter::HandleWindowFocus(
    [[maybe_unused]] GLFWwindow *window, int focused) {
...
void GLFWWindowCallbackAdapter::HandleWindowMoved(
    [[maybe_unused]] GLFWwindow *window, int xpos, int ypos) {
  EventBus::Publish<WindowMovedEvent>(xpos, ypos);
  // 避免过多的窗口移动日志，在调试时选择性启用
  // s_Logger->debug("Window moved to {}, {}", xpos, ypos);
}
void GLFWWindowCallbackAdapter::HandleMouseMove(
    [[maybe_unused]] GLFWwindow *window, double xpos, double ypos) {
  EventBus::Publish<MouseMoveEvent>(static_cast<float>(xpos),
...
void GLFWWindowCallbackAdapter::HandleMouseScroll(
    [[maybe_unused]] GLFWwindow *window, double xoffset, double yoffset) {
  EventBus::Publish<MouseScrollEvent>(xoffset, yoffset);
  // 避免过多的滚轮日志，在调试时选择性启用
  // s_Logger->debug("Mouse scrolled: xoffset={}, yoffset={}", xoffset,
  // yoffset);
}
void GLFWWindowCallbackAdapter::HandleKeyEvent(
    [[maybe_unused]] GLFWwindow *window, int key, int scancode, int action,
    int mods) {
  switch (action) {
    case GLFW_PRESS: {
      EventBus::Publish<KeyPressedEvent>(key, mods, false);  // 非重复按键
      s_Logger->trace("Key pressed: {} (scancode: {}, mods: {})", key, scancode,
                      mods);
      break;
    }
    case GLFW_RELEASE: {
    ...
    case GLFW_REPEAT: {
      ...
      s_Logger->trace("Key pressed repeatly: {} (scancode: {}, mods: {})", key,
                      scancode, mods);
      break;
    }
  }
}
void GLFWWindowCallbackAdapter::HandleCharInput(
...
GLFWWindowCallbackAdapter *GLFWWindowCallbackAdapter::GetAdapter(
    GLFWwindow *window) {
  auto *adapter = static_cast<GLFWWindowCallbackAdapter *>(
