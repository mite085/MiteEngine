...
void OpenGLPipeline::Initialize() {
  ...
  for (auto bindPoint :
       BindingPointManager::Get().GetAllocatedTextureBindings()) {
  ...
  // 添加ShadowMap Stage
  std::shared_ptr<OpenGLShader> shadowMapShader =
      ShaderCache::Get().GetOpenGLShader(
          FileSystem::GetAssetPath("shaders/shadowmap/shadowmap.vert.glsl")
              .string(),
          FileSystem::GetAssetPath("shaders/shadowmap/shadowmap.frag.glsl")
              .string());
  ...
  std::shared_ptr<OpenGLShader> gBufferShader =
      ShaderCache::Get().GetOpenGLShader(
          FileSystem::GetAssetPath("shaders/gbuffer/gbuffer.vert.glsl")
              .string(),
          FileSystem::GetAssetPath("shaders/gbuffer/gbuffer.frag.glsl")
              .string());
  AddStage(std::make_unique<GBufferStage>(), gBufferShader);

  // 添加Deferred Lighting Stage
  std::shared_ptr<OpenGLShader> deferredLightingShader =
      ShaderCache::Get().GetOpenGLShader(
          FileSystem::GetAssetPath(
              "shaders/lighting/deferred_lighting.vert.glsl")
              .string(),
          FileSystem::GetAssetPath(
              "shaders/lighting/deferred_lighting.frag.glsl")
              .string());
  AddStage(std::make_unique<DeferredLightingStage>(), deferredLightingShader);

  // 添加Forward Render Stage
  std::shared_ptr<OpenGLShader> forwardShader =
      ShaderCache::Get().GetOpenGLShader(
          FileSystem::GetAssetPath("shaders/forward/forward.vert.glsl")
              .string(),
          FileSystem::GetAssetPath("shaders/forward/forward.frag.glsl")
              .string());
  AddStage(std::make_unique<ForwardStage>(), forwardShader);

  // 添加Blend Render Stage
  std::shared_ptr<OpenGLShader> blendShader =
      ShaderCache::Get().GetOpenGLShader(
...
void OpenGLPipeline::BeginFrame() {
  ...
  RenderCommand::Get().SetViewport(
      0, 0, static_cast<uint32_t>(glm::max(m_PendingSize.x, 0.0f)),
      static_cast<uint32_t>(glm::max(m_PendingSize.y, 0.0f)));
...
void OpenGLPipeline::RenderScene(
