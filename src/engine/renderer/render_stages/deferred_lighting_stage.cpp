...
DeferredLightingStage::DeferredLightingStage()
    : RenderStage("DeferredLightingStage") {
...
void DeferredLightingStage::Execute(RenderContext &context) {
  if (!m_Initialized) {
    m_Logger->warn(
        "DeferredLightingStage executed but not properly initialized");
    return;
  }
  ...
  if (lightingShader->GetProgramId() == 0) {
    m_Logger->error(
        "DeferredLightingStage: lightingShader from context is not properly "
        "linked");
    return;
  }
  ...
  if (lightingTexture && lightingTexture->IsValid()) {
    ...
    RenderCommand::Get().PublishEventRuntimeTextureFinished(
        lightingTexture, "Deferred_Lighting_Combined");
...
void DeferredLightingStage::SetupLightingRenderState() {
  ...
  // 需要确保不同光源的贡献正确叠加
  std::static_pointer_cast<OpenGLRenderState>(m_LightingState)->blendSrc =
      GL_ONE;
  std::static_pointer_cast<OpenGLRenderState>(m_LightingState)->blendDst =
      GL_ONE;
}
...
void DeferredLightingStage::BindGBufferTextures(
  ...
  for (const auto &type : GBuffer::GetTextureTypes()) {
    ...
    if (texture && texture->IsValid()) {
      // 发布绑定命令
      RenderCommand::Get().BindRuntimeTexture(type, texture->GetHandle(),
                                              TextureTarget::TEXTURE_2D);
      m_Logger->trace("Bound G-Buffer texture: {} to unit {}",
                      GBuffer::GetTextureTypeName(type),
                      static_cast<int>(type));
    } else {
      m_Logger->warn("Missing G-Buffer texture: {}",
                     GBuffer::GetTextureTypeName(type));
    }
  }
}
...
void DeferredLightingStage::BindShadowMapTextures(RenderContext &context) {
  ...
  RuntimeTexturePtr directionalShadowTexture =
      context.GetShadowMapTexture(LightType::DIRECTIONAL);
  if (directionalShadowTexture && directionalShadowTexture->IsValid()) {
    RenderCommand::Get().BindRuntimeTexture(
        RuntimeTextureType::ShadowMap_Directional,
  ...
  RuntimeTexturePtr pointShadowTexture =
      context.GetShadowMapTexture(LightType::POINT);
  ...
  RuntimeTexturePtr spotShadowTexture =
      context.GetShadowMapTexture(LightType::SPOT);
...
void DeferredLightingStage::ValidateInputs(RenderContext &context) const {
  ...
  for (const auto &type : essentialTypes) {
    ...
    auto texture = context.GetRenderTarget(targetName);
    if (!texture || !texture->IsValid()) {
      m_Logger->error("Missing essential G-Buffer texture: {}",
                      GBuffer::GetTextureTypeName(type));
      throw std::runtime_error(
          "Deferred lighting stage missing essential G-Buffer textures");
    }
  }
}

void DeferredLightingStage::ValidateLightingFramebuffer(
  ...
  if (currentSize != viewportSize) {
    m_Logger->info("Resizing lighting FBO from {}x{} to {}x{}", currentSize.x,
                   currentSize.y, viewportSize.x, viewportSize.y);
    m_LightingFBO->Resize(
