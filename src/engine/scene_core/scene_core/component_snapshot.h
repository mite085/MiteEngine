...
class ComponentSnapshot : public ISnapshot {
 public:
   ...
   */
  size_t GetMemoryUsage() const override { return sizeof(DataT); }
  /**
   * @brief 获取快照描述信息
   * @return const char* 组件类型名称，用于调试和日志
   */
  const char *GetDescription() const override { return typeid(DataT).name(); }
  // ==================== 数据访问接口 ====================
  /**
   * @brief 获取关联的实体ID
   ...
   */
  const DataT &GetData() const { return m_snapshotData; }
