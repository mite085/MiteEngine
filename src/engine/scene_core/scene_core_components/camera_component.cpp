...
void CameraComponent::Zoom(float amount) { m_Camera->Zoom(amount); }
...
std::shared_ptr<Camera> &CameraComponent::GetCamera() { return m_Camera; }
...
const Camera &CameraComponent::GetSnapshotData() const { return *m_Camera; }
...
std::vector<std::type_index> CameraComponentSystem::GetSystemDependencies()
    const {
...
//   for (auto [m_Entity, component] : m_AllComponents) {
//     if (!component) {
//       LOG_ERROR("Empty camera component pointer in camera component
//       system!"); continue;
//     }
...
//     EventBus::Publish<MainCameraChangedEvent>(MainCameraChangedEvent(mainCamera,
//     *newMain));
//   }
//   else {
