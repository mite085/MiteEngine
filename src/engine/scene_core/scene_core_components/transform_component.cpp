...
TransformComponent::TransformComponent(const glm::vec3 position,
    ...
    : SnapshotComponentTraits(),
      m_Transform(std::make_shared<Transform>(position, rotation, scale, order))
{
}

TransformComponent::TransformComponent(const glm::mat4 &matrix, const Transform::EulerOrder order)
    : SnapshotComponentTraits(), m_Transform(std::make_shared<Transform>(matrix, order))
{
}
...
void TransformComponent::SetLocalTransform(std::function<void(Transform &)> transformOperator)
{
  if (m_Transform) {
    ...
    // 虽然 m_Transform.SetPosition、SetRotation等仅仅设置了m_MatrixDirty = true;
