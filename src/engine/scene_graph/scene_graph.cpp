...
SceneGraph::SceneGraph() {
  ...
  m_EventSubscriptions.SubscribeImmediate<EntityCreatedEvent>(
      BIND_DISPATCH_FN(OnEntityCreated), EventPriority::High);
  ...
  m_EventSubscriptions
      .SubscribeImmediate<ComponentAddedEvent<TransformComponent>>(
          BIND_DISPATCH_FN(OnTransformComponentAdded), EventPriority::Normal);
  m_EventSubscriptions
      .SubscribeImmediate<ComponentAddedEvent<BoundingVolumeComponent>>(
...
// ==================== 场景节点生命周期管理 ====================
std::shared_ptr<SceneNode> SceneGraph::CreateNode(SceneRegistry &registry,
                                                  Entity entity,
                                                  Entity parent) {
...
std::shared_ptr<SceneNode> SceneGraph::FindNodeByPath(
...
// ==================== 空间查询接口 ====================
std::vector<std::shared_ptr<SceneNode>> SceneGraph::FrustumCull(
...
std::vector<std::shared_ptr<SceneNode>> SceneGraph::Raycast(
    const Ray &ray) const {
  std::vector<std::shared_ptr<SceneNode>> results;
...
std::vector<std::shared_ptr<SceneNode>> SceneGraph::VolumeQuery(
    const BoundingVolume &volume) const {
  std::vector<std::shared_ptr<SceneNode>> results;
  m_SpatialPartition->VolumeQuery(volume, results);
  return results;
}
std::vector<std::shared_ptr<SceneNode>> SceneGraph::PointQuery(
...
void SceneGraph::TraverseVisible(
    std::function<bool(std::shared_ptr<SceneNode>)> callback,
    SceneNodeManager::TraversalType type) const {
...
void SceneGraph::Update(SceneRegistry &registry) {
  ...
  for (std::shared_ptr<SceneNode> node : m_NodeManager->GetLightNodes()) {
    if (node && registry.HasComponent<LightComponent>(node->GetEntity())) {
      ...
      lightTransforms.insert(
          std::make_pair(light.get(), node->GetWorldTransform()));
    }
  }
...
bool SceneGraph::IsEmpty() const { return m_NodeManager->IsEmpty(); }
...
void SceneGraph::OnTransformComponentAdded(
...
void SceneGraph::ProcessScheduledCreationsAndDestruction(
    SceneRegistry &registry) {
  // 1. 遍历待删除的实体队列(该步骤可能修改待创建队列,要先处理)
  for (auto entity : m_PendingDestroyNodes) {
    // 若存在于SceneNodeManager,正常销毁
    if (m_NodeManager->HasNode(entity)) DestroyNode(registry, entity);
  ...
  for (const auto &[entity, props] : m_PendingCreateNodes) {
    if (props.parent.IsValid() &&
        m_PendingCreateNodes.find(props.parent) != m_PendingCreateNodes.end()) {
  ...
  // 初始化：入度为0的节点入队
  for (const auto &[entity, props] : m_PendingCreateNodes) {
    if (inDegree[entity] == 0 && props.hasBoundingVolume &&
        props.hasTransform) {
    ...
    if (it != m_PendingCreateNodes.end()) {
      ...
      for (Entity child : childMap[entity]) {
        ...
        if (inDegree[child] == 0) {
          ...
          if (childIt != m_PendingCreateNodes.end() &&
              childIt->second.hasBoundingVolume &&
