...
class SimpleBVH : public SpatialPartition {
 ...
 private:
   ...
   * 2. 减少不必要的子树遍历
   */
  void RaycastBestFirst(BVHNode *root, const Ray &ray, std::vector<SceneNode *> &results) const;
  /**
   ...
   * @return 是否继续遍历
   */
  bool TraverseDFS(BVHNode *node, std::function<bool(SceneNode *)> callback) const;
  /**
   * @brief 递归调试绘制
   * @param node 当前节点
