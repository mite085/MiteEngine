...
CommandExecutionContext::CommandExecutionContext(CommandContextFlags contextFlags,
    ...
    : m_contextFlags(contextFlags),
      ...
      m_FailedCount(0)
{
  m_Logger = mite::LoggerSystem::CreateModuleLogger("Mite Command Executor Context" + name);
  m_Logger->debug("CommandExecutionContext '{}' created with flags: {}", name, static_cast<int>(contextFlags));
}
...
CommandExecutionContext::~CommandExecutionContext()
{
  ...
  if (!m_ExecutingCommands.empty()) {
    m_Logger->warn("CommandExecutionContext '{}' destroyed with {} executing commands",
                   m_name,
...
void CommandExecutionContext::AddContextFlags(CommandContextFlags flags)
{
  ...
  m_contextFlags = static_cast<CommandContextFlags>(oleFlags |= static_cast<uint32_t>(flags));
}
                                                 ...
                                                 const CommandHandle &handle) const
{
  ...
  if (!registry.IsCommandExecutable(handle)) {
    m_Logger->debug("Command '{}' is not in executable state", command->GetName());
    return false;
  }
...
// ==================== 命令执行跟踪接口实现 ====================
void CommandExecutionContext::RecordCommandExecutionStart(const CommandHandle &handle)
{
  if (!handle.IsValid())
    return;
  std::unique_lock lock(m_ExecutionMutex);
  ...
  if (m_ExecutingCommands.insert(handle).second) {
    m_ExecutingCount++;
    m_Logger->trace(
        "Command handle {} started execution in context '{}'", handle.ToString(), m_name);
  }
}
void CommandExecutionContext::RecordCommandExecutionComplete(const CommandHandle &handle,
                                                             const CommandResult &result)
{
  ...
  if (m_ExecutingCommands.erase(handle) > 0) {
    ...
    m_Logger->trace("Command handle {} completed in context '{}' with result: {}",
                    handle.ToString(),
                    m_name,
...
// ==================== 句柄管理接口实现 ====================
std::vector<CommandHandle> CommandExecutionContext::GetExecutingCommandHandles() const
{
  std::shared_lock lock(m_ExecutionMutex);
  return std::vector<CommandHandle>(m_ExecutingCommands.begin(), m_ExecutingCommands.end());
}
bool CommandExecutionContext::IsCommandExecuting(const CommandHandle &handle) const
{
  std::shared_lock lock(m_ExecutionMutex);
