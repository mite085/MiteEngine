...
CommandHandle CommandRegistry::CreateCommand(std::type_index typeIndex) {
  ...
  if (!command) {
    m_Logger->error("Failed to create command instance for type: {}",
                    typeIndex.name());
    return CommandHandle();
  }
...
CommandPtr CommandRegistry::AcquireCommand(const CommandHandle &handle,
  ...
  if (expectedType != std::type_index(typeid(void)) &&
    ...
    m_Logger->error("Command type mismatch. Expected: {}, Actual: {}",
                    expectedType.name(), it->second.type.name());
    return nullptr;
  }
  ...
  m_Logger->debug("Acquired command: {} ({})", command->GetName(),
                  handle.ToString());
  return command;
}
  ...
  if (it == m_commandInstances.end()) {
    ...
    m_commandInstances.try_emplace(
        ...
        CommandExecutionState::
            PENDING  // 新实例无视state输入，直接设定为Pending，确保状态管理正确
    );
  } else {
    // 替换现有实例
    if (it->second.command && !it->second.isAcquired) {
      m_Logger->warn("Handle {} already has associated command",
                     handle.ToString());
      return false;
    }
...
CommandHandle CommandRegistry::PreAllocateHandle() {
  ...
  std::unique_lock lock(m_instancesMutex);
  // 预分配时命令对象为空
  m_commandInstances.try_emplace(handle, nullptr,
                                 std::type_index(typeid(void)));

  m_Logger->debug("Pre-allocated handle: {}", handle.ToString());
  return handle;
}
bool CommandRegistry::AssociateCommand(const CommandHandle &handle,
                                       CommandPtr command) {
  if (!handle.IsValid() || !command) {
    return false;
  }
...
bool CommandRegistry::ReleaseCommand(const CommandHandle &handle) {
  ...
  m_Logger->debug("Released command: {} ({})", it->second.command->GetName(),
                  handle.ToString());
  m_commandInstances.erase(it);
  return true;
}
// ==================== 状态管理接口实现 ====================
bool CommandRegistry::SetCommandState(const CommandHandle &handle,
  ...
  if (!ValidateStateTransition(it->second.state, state)) {
    m_Logger->warn("Invalid state transition from {} to {} for handle {}",
                   ...
                   CommandResult::StateToString(state), handle.ToString());
    return false;
  }
  it->second.state = state;
  m_Logger->trace("Command handle {} state changed to {}", handle.ToString(),
                  CommandResult::StateToString(state));
  return true;
}
CommandExecutionState CommandRegistry::GetCommandState(
...
bool CommandRegistry::IsCommandExecutable(const CommandHandle &handle) const {
  ...
  return state == CommandExecutionState::PENDING ||
         ...
         state == CommandExecutionState::UNDONE ||
         state == CommandExecutionState::REDONE;
}
bool CommandRegistry::IsCommandExecuting(const CommandHandle &handle) const {
  CommandExecutionState state = GetCommandState(handle);

  // 正在执行、正在撤销、正在重做均是为Executing
  return state == CommandExecutionState::EXECUTING ||
         state == CommandExecutionState::UNDOING ||
...
bool CommandRegistry::IsCommandCompleted(const CommandHandle &handle) const {
  ...
  return state == CommandExecutionState::SUCCEEDED ||
...
// ==================== 类型信息查询接口实现 ====================
std::string CommandRegistry::GetCommandTypeName(
    std::type_index typeIndex) const {
  std::shared_lock lock(m_typesMutex);
  auto it = m_commandTypes.find(typeIndex);
  return it != m_commandTypes.end() ? it->second.typeName : "";
}
CommandCategory CommandRegistry::GetCommandCategory(
    std::type_index typeIndex) const {
  std::shared_lock lock(m_typesMutex);
  auto it = m_commandTypes.find(typeIndex);
  return it != m_commandTypes.end() ? it->second.category
                                    : COMMAND_CATEGORY_NONE;
}
CommandPriority CommandRegistry::GetCommandDefaultPriority(
    std::type_index typeIndex) const {
  std::shared_lock lock(m_typesMutex);
  auto it = m_commandTypes.find(typeIndex);
  return it != m_commandTypes.end() ? it->second.defaultPriority
                                    : CommandPriority::NORMAL;
}

// ==================== 批量查询接口实现 ====================
CommandRegistry::CommandTypeList
CommandRegistry::GetRegisteredCommandTypeIndices() const {
  std::shared_lock lock(m_typesMutex);
...
// ==================== 内部辅助方法 ====================
bool CommandRegistry::ValidateStateTransition(CommandExecutionState from,
                                              CommandExecutionState to) {
  // 定义允许的状态转换
  static const std::unordered_map<CommandExecutionState,
                                  std::unordered_set<CommandExecutionState>>
      validTransitions = {
          {CommandExecutionState::PENDING,
           {CommandExecutionState::
                EXECUTING}},  // 待执行状态仅可以转换为执行状态
