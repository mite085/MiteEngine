...
class PBRMaterialTemplate : public Material {
 public:
   ...
   */
  explicit PBRMaterialTemplate(std::shared_ptr<Shader> shader,
 ...
 protected:
  // ---- 默认参数 ----
  std::shared_ptr<Shader> m_Shader;  // 关联的PBR着色器程序
...
class TransparentMaterialTemplate : public PBRMaterialTemplate {
 public:
  explicit TransparentMaterialTemplate(std::shared_ptr<Shader> shader, float defaultAlpha = 0.5f);
